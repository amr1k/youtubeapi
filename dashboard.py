...
try:
  rawClientSecret = Secrets.access_secret_version(projectId, youtubeClientSecret , 'latest')
except:
  print('error loading youtube-client-secret from secret manager')

try:
  rawYoutubeCreds = Secrets.access_secret_version(projectId, youtubeCredentials, 'latest')
except:
...
if __name__ == '__main__':
  # - If there is not secret, the user has not authorised oAuth to call the API on behalf of the user
  ...
  if(rawYoutubeCreds == None):
    ...
    resp = Secrets.add_secret_version(projectId, youtubeCredentials, clientSecret.to_json())
    rawYoutubeCreds = clientSecret

  # Convert the content of the secret to Google oauth credentials object
  print('convert youtube credentials from secrets manager object')
  ...
  while(1):
    ...
    if(youtubeCredentials.expired):
      ...
      Secrets.add_secret_version(projectId, youtubeCredentials, youtubeCredentials.to_json())
